package com.samuelangan.mycompagny.service;

import com.samuelangan.mycompagny.domain.*;
import com.samuelangan.mycompagny.repository.*;
import com.samuelangan.mycompagny.security.OrderConstants;
import com.samuelangan.mycompagny.service.dto.OrderDTO;
import com.samuelangan.mycompagny.web.rest.errors.BadRequestAlertException;
import java.util.*;
...
public class OrderService {
    ...
    private void validateOrderDependencies(Order order) {
        ...
        userRepository.findById(userId).orElseThrow(() -> new BadRequestAlertException("User not found", "Order Saving", "UserNotFound"));

        eventRepository
            .findById(eventId)
            .orElseThrow(() -> new BadRequestAlertException("Event not found", "Order Saving", "EventNotFound"));
    }
    ...
    private void validateAndSetBoxCatalog(OrderItem orderItem) {
        ...
        boxCatalogRepository
            .findById(boxCatalog.getId())
            .orElseThrow(() -> new BadRequestAlertException("BoxCatalog not found", "Order Saving", "NotFoundBoxCatalog"));
    }
    ...
    private Order createUpdateProcessOrder(OrderDTO orderDTO, boolean isUpdate, boolean isProcess) {
        ...
        if (isProcess) {
            boolean allItemsRetrieved = order.getOrderItems().stream().allMatch(OrderItem::getOrderItemRetrieved);
    ...
    public Page<OrderDTO> searchAllOrder(String search, Pageable pageable) {
        LookupOperation lookupOperationEVT = LookupOperation.newLookup().from("event").localField("event").foreignField("_id").as("evt");

        LookupOperation lookupOperationUSR = LookupOperation.newLookup().from("user").localField("user").foreignField("_id").as("usr");

        LookupOperation lookupOperationOI = LookupOperation.newLookup()
            .from("order_item")
            .localField("order_items")
            .foreignField("_id")
            .as("orderitems");

        LookupOperation lookupOperationBC = LookupOperation.newLookup()
            .from("box_catalog")
            .localField("orderitems.box_catalog")
            .foreignField("_id")
            .as("boxcatalog");

        LookupOperation lookupOperationCat = LookupOperation.newLookup()
            .from("catalog")
        ...
        final Long totalCount = Optional.ofNullable(
            mongoTemplate.aggregate(aggregationCount, "order", Document.class).getUniqueMappedResult()
        )
