...
import com.samuelangan.mycompagny.repository.ApplicationLogRepository;
import com.samuelangan.mycompagny.web.rest.vm.ApplicationLogVM;
...
public class ApplicationLogService {
    ...
    public Page<ApplicationLogVM> searchAllBox(String search, Pageable pageable) {
        LookupOperation lookupOperationWH = LookupOperation.newLookup().from("user").localField("user").foreignField("_id").as("user");
        ...
        Aggregation aggregationCount = Aggregation.newAggregation(lookupOperationWH, matchOperation, Aggregation.count().as("count"));
        final Long totalCount = Optional.ofNullable(
            mongoTemplate.aggregate(aggregationCount, "log", Document.class).getUniqueMappedResult()
        )
