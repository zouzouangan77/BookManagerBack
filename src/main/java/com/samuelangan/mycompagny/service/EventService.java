...
import com.samuelangan.mycompagny.service.dto.EventDTO;
...
public class EventService {
    ...
    public EventService(EventRepository eventRepository, OrderRepository orderRepository, MongoTemplate mongoTemplate) {
    ...
    private Event createUpdateEvent(EventDTO eventDTO, Boolean isUpdate) {
        ...
        if (isUpdate) {
            if (StringUtils.isBlank(id) || eventRepository.findById(id).isEmpty()) {
                throw new BadRequestAlertException("Event '" + event.getId() + "' not found", "Event Updating", "NotFound Event ");
            }
            ...
            if (eventRepository.findOneByNameIgnoreCase(event.getName()).isPresent()) {
                throw new BadRequestAlertException("Event '" + event.getName() + "' Already Exists", "Event Saving", "Already Exists");
            }
        }
    ...
    public Page<EventDTO> searchAllEvent(String search, Pageable pageable) {
        ...
        Aggregation aggregationCount = Aggregation.newAggregation(matchOperation, Aggregation.count().as("count"));
        ...
        final Long totalCount = Optional.ofNullable(
            mongoTemplate.aggregate(aggregationCount, "event", Document.class).getUniqueMappedResult()
        )
