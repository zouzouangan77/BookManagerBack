package com.samuelangan.mycompagny.web.rest;
...
import com.samuelangan.mycompagny.IntegrationTest;
...
class BookmanagerbackKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bookmanagerback-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/bookmanagerback-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/bookmanagerback-kafka/unregister"));
                return;
            }
        }
